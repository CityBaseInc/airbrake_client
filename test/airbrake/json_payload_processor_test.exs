if Code.ensure_loaded?(:json) do
  defmodule Airbrake.JsonPayloadProcessorTest do
    use ExUnit.Case, async: true

    alias Airbrake.JsonPayloadProcessor
    ...
    describe "process_params/2" do
      test "destructures structs, filters, and converts tuples to lists" do
        assert JsonPayloadProcessor.process_params(
                 %Team{
                   members: [
    ...
    describe "process_headers/2" do
      test "filters matching keys" do
        assert JsonPayloadProcessor.process_headers(
    ...
    describe "encode!/1" do
      test "encodes a map to JSON" do
        assert JsonPayloadProcessor.encode!(%{"key" => "value"}) == ~s/{"key":"value"}/
      end
    end
  end
end
