defmodule Airbrake.PoisonPayloadProcessorTest do
  use ExUnit.Case, async: true

  alias Airbrake.PoisonPayloadProcessor
  ...
  describe "process_params/2" do
    test "destructures structs, filters, and converts tuples to lists" do
      assert PoisonPayloadProcessor.process_params(
               %Team{
                 members: [
  ...
  describe "process_headers/2" do
    test "filters matching keys" do
      assert PoisonPayloadProcessor.process_headers(
  ...
  describe "encode!/1" do
    test "encodes a map to JSON" do
      assert PoisonPayloadProcessor.encode!(%{"key" => "value"}) == ~s/{"key":"value"}/
    end
  end
end
